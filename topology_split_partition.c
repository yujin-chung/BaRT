...
void
split_initialize_l (split_partition *bpt, int label_id, int *n_leaves)
{
  bpt->l = (1LL << label_id);
  bpt->n = 1;
}

void split_OR_l (split_partition *res, const split_partition *b1, const split_partition *b2, int *n_leaves)
{
  res->l = b1->l | b2->l;
  res->n = b1->n + b2->n;
}
...
{
	int i, j, k = split->n_u2, best_j, best_n;

	split->n_disagree = 0;
	for (i=0; (i < split->n_u1) && (k); i++) {
		best_j = 0; best_n = split->n_leaves;
		for (j=0; (j < k); j++) {
...
*/
			...
			if (split->d_ptr[split->n_disagree]->n <= split->n_active/2)	
				split->n_disagree++;
		}
	}
}
