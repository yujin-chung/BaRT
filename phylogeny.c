...
/*----------------------------------------------
  ...
  It replaces "new_phylogeny"
----------------------------------------------- */
phylogeny_gibbs
new_phylogeny_gibbs (int nchar, int ntax, int ncycles)
{
  phylogeny_gibbs p;
  int i;
  
  p = (phylogeny_gibbs) biomc2_malloc (sizeof (struct phylogeny_gibbs));
  p->nchar  = nchar;
  p->ntax   = ntax;
  p->nnodes = ((2*p->ntax) - 2);
  ...
  phylogeny_gibbs does not have "lambda" as member.
  Instead chain_gibbs has "lambda" as member.
  fixit: in some other functions, we need to define chain->lambda=0.;
---------------------------------------- */
  //  p->lambda = 0.; 
  ...
  p->l = (node_likelihood*) biomc2_malloc (p->nnodes * sizeof (node_likelihood));
	
  p->l_ptr = (node_likelihood*) biomc2_malloc ((p->nnodes - p->ntax) * sizeof (node_likelihood));
	
  /* allocate memory for likelihood vectors */
  for (i=0; i < p->ntax; i++)         
    p->l[i] = new_node_likelihood (p->nchar, 1); 
  for (i=p->ntax; i < p->nnodes; i++) 
    p->l[i] = new_node_likelihood (p->nchar, ncycles + 3); 

  for (i=p->ntax; i < p->nnodes; i++) 
    p->l_ptr[i - p->ntax] = p->l[i];
...
/*------------------------------------
  By Yujin Chung on 2/6/2013:
  It replaces "del_phylogeny".
-------------------------------------- */
void 
del_phylogeny_gibbs (phylogeny_gibbs p)
{
...
/*--------------------------------
  By Yujin Chung on 2/6/2013:
  It replaces "new_phylogeny_from_nexus".
----------------------------------- */
phylogeny_gibbs
new_phylogeny_from_nexus_gibbs (nexus_alignment a, double *siteweight, int *idx, int n_idx, int ncycles)
{
  phylogeny_gibbs p;
  int i;

  p = new_phylogeny_gibbs (n_idx, a->ntax, ncycles);
  for (i=0; i < n_idx; i++) 
    p->site_weight[i] = siteweight[ idx[i] ];
  for (i=0; i < a->ntax; i++) 
    store_likelihood_at_leaf (p->l[i], a->character[i], idx, n_idx);
  return p;
}
...
/*------------------------------------------------
  By Yujin Chung on 2/6/2013:
  It replaces "average_rate_distance_method".
------------------------------------------------- */
double
average_rate_distance_method_gibbs (phylogeny_gibbs phy)
{
  int i1, i2, b1, b2, pos;
  int e_1=0, d_1=0;
  double e_2 = 0., d_2=0.;
...
/*--------------------------
  By Yujin Chung on 2/6/2013:
  It replaces "update_equilibrium_frequencies"
----------------------------- */
void
update_equilibrium_frequencies_gibbs (double *pi, phylogeny_gibbs phy)
{
  int i, b, pos;
  double **seq;
  ...
  for (i=0; i < phy->ntax; i++) {
    ...
    for (pos=0; pos < phy->nchar; pos++)
      {
	// phy->nchar takes the segment size (in number of distinct patterns, not sites) -- YC
	for (b=0; b < 4; b++)
	  pi[b] += (phy->site_weight[pos] * seq[pos][b]); // phy->site_weight takes the frequency of each site pattern -- YC
      }
  }
}
