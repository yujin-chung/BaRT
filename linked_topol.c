...
/*------------------------------------
  ...
  It replaces "shuffle_topology".
---------------------------------------- */
void shuffle_topology_gibbs (topology t, phylogeny_gibbs *segment, linked_topol tp, split_space split);
...
//	int i, n_shuffle = biomc2_rng_uniform_int (random_number, 3);
	...
	/*------------------------------------
	  By Yujin Chung on 2/7/2013
	  We need to revise the below part: "spr_apply_random_move_spr"
	---------------------------------------- */
	for (i=0; i <= n_shuffle; i++) 
	  spr_apply_random_move_spr (tp->bkp[BKPrandom], true);

	copy_topology_from_topology_mapping (t, tp->bkp[BKPrandom], split);
	swap_likelihood_gibbs (t, segment, tp->bkp[BKPrandom]);
}


/*------------------------------------
  By Yujin Chung on 2/7/2013
  It replaces "pre_add_remove_topology".
---------------------------------------- */
void pre_add_remove_topology_gibbs (topology t, phylogeny_gibbs *segment, linked_topol tp)
{
  clear_topology (t);
  copy_topology_from_topology (tp->bkp[BKPcopy], t);
  link_current_to_accepted_gibbs (t, segment);
}


/*------------------------------------
  By Yujin Chung on 2/7/2013
  It replaces "add_topology_forward".
---------------------------------------- */
void add_topology_forward_gibbs (topology t, phylogeny_gibbs *segment, linked_topol tp, split_space split)
{
  /* ( [t ] ) ==> ( [t->prev] [t ] ) segments 
	 * ( [T1] ) ==> ( [  T1*  ] [T1] ) topology */
  ...
  link_current_to_accepted_gibbs (t->prev, segment);
...
/*------------------------------------
  By Yujin Chung on 2/7/2013
  It replaces "accept_add_topology_forward".
---------------------------------------- */
void
accept_add_topology_forward_gibbs (topology t, phylogeny_gibbs *segment)
{
  clear_topology (t->prev);
  link_accepted_to_current_gibbs (t->prev, segment);
  sum_segment_likelihood_gibbs (t, segment);
  t->likelihood_accepted = t->likelihood_current;
}

/*------------------------------------
  By Yujin Chung on 2/7/2013
  It replaces "reject_add_topology_forward".
  
  Questions:
  -- Is argument "segment" necessary?
---------------------------------------- */
void
reject_add_topology_forward_gibbs (topology t, phylogeny_gibbs *segment, linked_topol tp)
{
...
/*------------------------------------
  By Yujin Chung on 2/7/2013
  It replaces "add_topology_backward".
---------------------------------------- */
void
add_topology_backward_gibbs (topology t, phylogeny_gibbs *segment, linked_topol tp, split_space split)
{
  /* ( [t ] ) ==> ( [t ] [t->next] ) 
   * ( [T1] ) ==> ( [T1] [  T1*  ] ) */
  int i, mid = t->last_segment - t->first_segment;
  ...
  link_current_to_accepted_gibbs (t->next, segment);
...
/*------------------------------------
  By Yujin Chung on 2/7/2013
  It replaces "accept_add_topology_backward".
---------------------------------------- */
void
accept_add_topology_backward_gibbs (topology t, phylogeny_gibbs *segment)
{
  clear_topology (t->next);
  link_accepted_to_current_gibbs (t->next, segment);
  sum_segment_likelihood_gibbs (t, segment);
  t->likelihood_accepted = t->likelihood_current;
}
...
/*------------------------------------
  By Yujin Chung on 2/7/2013
  It replaces "reject_add_topology_backward".
---------------------------------------- */
void
reject_add_topology_backward_gibbs (topology t, phylogeny_gibbs *segment, linked_topol tp)
{
...
/*------------------------------------
  By Yujin Chung on 2/7/2013
  It replaces "remove_topology_forward".
---------------------------------------- */
void
remove_topology_forward_gibbs (topology t, phylogeny_gibbs *segment, linked_topol tp, split_space split) 
{
	/* ( [t->prev] ) ( [t] )  ==> ( [t->prev] ) ( [t->prev] ) */
  copy_topology_from_topology_mapping (t, t->prev, split);
  swap_likelihood_gibbs (t, segment, t->prev);
}

/*------------------------------------
  By Yujin Chung on 2/7/2013
  It replaces "accept_remove_topology_forward".
---------------------------------------- */
void
accept_remove_topology_forward_gibbs (topology t, phylogeny_gibbs *segment, linked_topol tp)
{
  link_accepted_to_current_gibbs (t, segment);
  clear_topology (t);
...
---------------------------------------- */
void
reject_remove_topology_forward_gibbs (topology t, phylogeny_gibbs *segment, linked_topol tp)
{
  unswap_likelihood_gibbs (t, segment, t->prev);
...
/*------------------------------------
  By Yujin Chung on 2/7/2013
  It replaces "remove_topology_backward".
---------------------------------------- */
void
remove_topology_backward_gibbs (topology t, phylogeny_gibbs *segment, linked_topol tp, split_space split) 
{
	/* ( [t] ) ( [t->next] ) ==> ( [t->next] ) ([t->next] ) */
  copy_topology_from_topology_mapping (t, t->next, split);
  swap_likelihood_gibbs (t, segment, t->next);
}


/*------------------------------------
  By Yujin Chung on 2/7/2013
  It replaces "accept_remove_topology_backward".
---------------------------------------- */
void
accept_remove_topology_backward_gibbs (topology t, phylogeny_gibbs *segment, linked_topol tp)
{
	link_accepted_to_current_gibbs (t, segment);
	clear_topology (t);
...
/*------------------------------------
  By Yujin Chung on 2/7/2013
  It replaces "reject_remove_topology_backward".
---------------------------------------- */
void
reject_remove_topology_backward_gibbs (topology t, phylogeny_gibbs *segment, linked_topol tp)
{
  unswap_likelihood_gibbs (t, segment, t->next);
  copy_topology_from_topology (t, tp->bkp[BKPcopy]);
  clear_topology (t);
}
...
/*------------------------------------
  By Yujin Chung on 2/7/2013
  It replaces "shift_topology_forward".
---------------------------------------- */
void
shift_topology_forward_gibbs (topology t, phylogeny_gibbs *segment, linked_topol tp, split_space split)
{
  ...
  swap_likelihood_gibbs (t->prev, segment, t->prev->prev);
  link_current_to_accepted_gibbs (t->prev, segment);
}

/*------------------------------------
  By Yujin Chung on 2/7/2013
  It replaces "accept_shift_topology_forward".
---------------------------------------- */
void
accept_shift_topology_forward_gibbs (topology t, phylogeny_gibbs *segment, linked_topol tp)
{
	link_accepted_to_current_gibbs (t->prev, segment);
	sum_segment_likelihood_gibbs (t, segment);
	t->likelihood_accepted = t->likelihood_current;
...
/*------------------------------------
  By Yujin Chung on 2/7/2013
  It replaces "reject_shift_topology_forward".
---------------------------------------- */
void
reject_shift_topology_forward_gibbs (topology t, phylogeny_gibbs *segment, linked_topol tp)
{
  unswap_likelihood_gibbs (t->prev, segment, t->prev->prev);
  t->first_segment = t->prev->first_segment;
  remove_topology (t->prev, tp);
}
...
/*------------------------------------
  By Yujin Chung on 2/7/2013
  It replaces "shift_topology_backward".
---------------------------------------- */
void
shift_topology_backward_gibbs (topology t, phylogeny_gibbs *segment, linked_topol tp, split_space split)
{
  ...
  swap_likelihood_gibbs (t->next, segment, t->next->next);
  link_current_to_accepted_gibbs (t->next, segment);
}
...
/*------------------------------------
  By Yujin Chung on 2/7/2013
  It replaces "accept_shift_topology_backward".
---------------------------------------- */
void
accept_shift_topology_backward_gibbs (topology t, phylogeny_gibbs *segment, linked_topol tp)
{
  link_accepted_to_current_gibbs (t->next, segment);
  sum_segment_likelihood_gibbs (t, segment);
  t->likelihood_accepted = t->likelihood_current;
...
/*------------------------------------
  By Yujin Chung on 2/7/2013
  It replaces "reject_shift_topology_backward".
---------------------------------------- */
void
reject_shift_topology_backward_gibbs (topology t, phylogeny_gibbs *segment, linked_topol tp)
{
  unswap_likelihood_gibbs (t->next, segment, t->next->next);
  t->last_segment = t->next->last_segment;
  remove_topology (t->next, tp);
}
...
/*------------------------------------
  By Yujin Chung on 2/7/2013
  It replaces "swap_likelihood".
---------------------------------------- */
void
swap_likelihood_gibbs (topology old, phylogeny_gibbs *segment, topology t)
{
  int i, j, k;
...
/*------------------------------------
  By Yujin Chung on 2/7/2013
  It replaces "unswap_likelihood".
---------------------------------------- */
void
unswap_likelihood_gibbs (topology old, phylogeny_gibbs *segment, topology t)
{
	int i, j, k;
...
/*------------------------------------
  By Yujin Chung on 2/7/2013
  It replaces "link_current_to_accepted".
---------------------------------------- */
void
link_current_to_accepted_gibbs (topology t, phylogeny_gibbs *segment)
{
  int i, j;
...
/*------------------------------------
  By Yujin Chung on 2/7/2013
  It replaces "link_accepted_to_current".
---------------------------------------- */
void
link_accepted_to_current_gibbs (topology t, phylogeny_gibbs *segment)
{
  int i, j;
  for (i=t->first_segment; i <= t->last_segment; i++) {
...
/*------------------------------------
  By Yujin Chung on 2/7/2013
  It replaces "sum_segment_likelihood".
---------------------------------------- */
void
sum_segment_likelihood_gibbs (topology t, phylogeny_gibbs *segment)
{
  int i;
  t->likelihood_current = 0.; 
  for (i=t->first_segment; i <= t->last_segment; i++)
