...
/*------------------------------------
  ...
  New functions are created because of
  new data type chain_data_gibbs and phylogeny_gibbs
---------------------------------------- */
void gibbsPrior_dist_term_gibbs (double *result, chain_data_gibbs chain, phylogeny_gibbs p, int *new_dist);
void gibbsPrior_dist_term_awadhi_gibbs (double *result, chain_data_gibbs chain, phylogeny_gibbs p, int *new_dist, double *kT1, double *kT2);
void gibbsPrior_dist_term_no_ratio_gibbs (double *result, chain_data_gibbs chain, phylogeny_gibbs p);

int mcmc_topol_update_gibbs (chain_data_gibbs chain, topology t);
int mcmc_topol_update_shuffle_gibbs (chain_data_gibbs chain, topology t);
void al_awadhi_minisampler_gibbs (chain_data_gibbs chain, topology t, acceptance_frequency *afreq);
...
/*------------------------------------
 ...
 By Yujin Chung on 3/6/2013:
  fixit: double-check that p->lambda (used in biomc2) is the same as chain->lambda. That is, only tree is updated and the value of lambda is fixed.
---------------------------------------- */
void gibbsPrior_dist_term_gibbs (double *result, chain_data_gibbs chain, phylogeny_gibbs p, int *new_dist)
{  
  (*result)	= -chain->lambda * (double)((*new_dist) - p->dist) ; 
...
/*------------------------------------
  By Yujin Chung on 2/7/2013
  It replaces "poisson_dist_term_awadhi".
  weight w and related parameters were removed
  By Yujin Chung on 3/4/2013:
  In BaRT, lambda is the same across the segments and it is a member of chain_data_gibbs
 By Yujin Chung on 3/6/2013:
...
---------------------------------------- */
// By Yujin Chung on 3/6/2013
// to calculate the ratio of prior Gibbs distribution with old RF-distance (p->dist) and new RF-distance (new_dist)
void gibbsPrior_dist_term_awadhi_gibbs (double *result, chain_data_gibbs chain, phylogeny_gibbs p, int *new_dist, double *kT1, double *kT2)
{

  (*result) = -chain->lambda*(double)(*new_dist)* (*kT1) + chain->lambda*(double)(p->dist)*(*kT2);
  /*--------------
...
/*------------------------------------
  By Yujin Chung on 2/7/2013
  It replaces "poisson_dist_no_ratio".
  weight w and related parameters were removed
  In BaRT, lambda is the same across the segments and it is a member of chain_data_gibbs
  By Yujin Chung on 3/6/2013:
  fixit: double-check that p->lambda (used in biomc2) is the same as chain->lambda. That is, only tree is updated and the value of lambda is fixed.
  fixit: after modifing this function, the large number of breakpoints were sampled! need to fixit!
---------------------------------------- */
void gibbsPrior_dist_term_no_ratio_gibbs (double *result, chain_data_gibbs chain, phylogeny_gibbs p)
{
   (*result)= -(chain->lambda * (double)(p->dist)); 
...
int mcmc_topol_update_gibbs (chain_data_gibbs chain, topology t)
{
  ...
  if (t->prev) {
    ...
    if (d_lp > chain->max_distance) {
      ...
      gibbsPrior_dist_term_gibbs (&sum_l, chain, chain->segment[t->prev->last_segment], &d_lp);
  }
  if (t->next) {
    ...
    if (d_rp > chain->max_distance) { 
      ...
      gibbsPrior_dist_term_gibbs (&sum_r, chain, chain->segment[t->last_segment], &d_rp);
  }
  
  chain->ln_likelihood_moved_branches_gibbs (chain->segment, t);
  //	if (debug_topol (t, chain)) printf ("topol_update\n");
  
  acceptance = chain->kT * ( (t->likelihood_proposal - t->likelihood_current) + sum_l + sum_r);
  
  if (chain->acceptance_probability_gibbs (chain, &acceptance, &transition)) {
    accept_likelihood_moved_branches_gibbs (chain->segment, t);
    
    if (t->prev) 
      chain->segment[t->prev->last_segment]->dist = d_lp;
    if (t->next) 
      chain->segment[t->last_segment]->dist = d_rp;
    return 1;
  }
  else { 
    spr_apply_move_at_nodes (t, t->undo_prune, t->undo_regraft, true);
...
/*------------------------------------
  By Yujin Chung on 2/7/2013
  It replaces "mcmc_topol_update_shuffle"
---------------------------------------- */
int
mcmc_topol_update_shuffle_gibbs (chain_data_gibbs chain, topology t)
{
  double sum_l = 0., sum_r = 0., acceptance, transition = 0.;
  ...
  swap_likelihood_gibbs (t, chain->segment, chain->topol->bkp[BKPshuffle_random]);
...
//	if (debug_topol (t, chain)) printf ("update_topol_shuffle\n");
  ...
  if (chain->acceptance_probability_gibbs (chain, &acceptance, &transition)) {
    ...
    unswap_likelihood_gibbs (t, chain->segment, chain->topol->bkp[BKPshuffle_random]);
...
/*------------------------------------
  By Yujin Chung on 2/7/2013
  It replaces "mcmc_topol_update_outside".
---------------------------------------- */
// By Yujin Chung on 3/6/2013:
// it is called to update tree topology
void mcmc_topol_update_outside_gibbs (chain_data_gibbs chain, topology t, acceptance_frequency *afreq)
{
  ...
  link_current_to_accepted_gibbs (t, chain->segment);
  
	/* pi_beta(x)/pi(x) */
  if (t->prev) {
    d_[0] = chain->segment[t->prev->last_segment]->dist;
    gibbsPrior_dist_term_awadhi_gibbs (&sum_[0], chain, chain->segment[t->prev->last_segment], &d_[0], &(temperature), &(chain->kT));
  }
  if (t->next) {
    d_[1] = chain->segment[t->last_segment]->dist;
    gibbsPrior_dist_term_awadhi_gibbs (&sum_[1], chain, chain->segment[t->last_segment], &d_[1], &(temperature), &(chain->kT));
  }
  ...
  for (i=0; i < chain->n_mini; i++) 
    if ( mcmc_topol_update_gibbs (chain, t)) {
  ...
  if (t->prev) {
    ...
    gibbsPrior_dist_term_awadhi_gibbs (&sum_[0], chain, chain->segment[t->prev->last_segment], &d_[2], &(chain->kT), &(temperature) );
  }
  if (t->next) {
    d_[3] = chain->segment[t->last_segment]->dist;
    gibbsPrior_dist_term_awadhi_gibbs (&sum_[1], chain, chain->segment[t->last_segment], &d_[3], &(chain->kT), &(temperature) );
  }
  acceptance += ( ((chain->kT - temperature) * t->likelihood_current) + sum_[0] + sum_[1] );
  
  if (chain->acceptance_probability_gibbs (chain, &acceptance, &transition)) {
    afreq->accept[FREQtopol]++;
    link_accepted_to_current_gibbs (t, chain->segment);
  }
  else {
    if (t->prev) 
...
/*------------------------------------
  By Yujin Chung on 2/7/2013
  It replaces "al_awadhi_minisampler".
---------------------------------------- */
void al_awadhi_minisampler_gibbs (chain_data_gibbs chain, topology t, acceptance_frequency *afreq)
{
  int i;
...
/*------------------------------------
  By Yujin Chung on 2/7/2013
  It replaces "proposal_update_topol_birth_forward".
---------------------------------------- */
void proposal_update_topol_birth_forward_gibbs (topology t, chain_data_gibbs chain, acceptance_frequency *afreq, double *log2)
{
#ifdef DEBUG
    printf("\nREMOVE: calling proposal_update_topol_birth_forward_gibbs (t, chain, afreq, &log2)\n");
#endif // DEBUG
  ...
  add_topology_forward_gibbs (t, chain->segment, chain->topol, chain->split);
  
  /* store new and old (if rejected in the end) dist info and prior ratio terms */
  if (t->prev->prev) {
    #ifdef DEBUG
    printf("REMOVE: Calling dSPR_topology_l (t->prev, t->prev->prev, chain->split, &(chain->max_dist_split));\n");
#endif //DEBUG    
    d_[0] = dSPR_topology_l (t->prev, t->prev->prev, chain->split, &(chain->max_dist_split));
    #ifdef DEBUG
    printf("REMOVE: Done with dSPR_topology_l (t->prev, t->prev->prev, chain->split, &(chain->max_dist_split));\n");
#endif //DEBUG    
    if (d_[0] > chain->max_distance) { 
      reject_add_topology_forward_gibbs (t, chain->segment, chain->topol); 
      return; }
    gibbsPrior_dist_term_awadhi_gibbs (&sum_[0], chain, chain->segment[t->prev->prev->last_segment], &d_[0], &(temperature), &(chain->kT));
    d_[2] = chain->segment[t->prev->prev->last_segment]->dist;
  }
  
  d_[1] = dSPR_topology_l (t, t->prev, chain->split, &(chain->max_dist_split));
  if (d_[1] > chain->max_distance) { 
    reject_add_topology_forward_gibbs (t, chain->segment, chain->topol); 
    return; }
  gibbsPrior_dist_term_awadhi_gibbs (&sum_[1], chain, chain->segment[t->prev->last_segment],  &d_[1], &(temperature), &(chain->kT));
  d_[3] = chain->segment[t->prev->last_segment]->dist;

  /* likelihood ratio of dimension transition */
  /* pi_beta(x')/pi(x) */
  chain->ln_likelihood_moved_branches_gibbs (chain->segment, t->prev);
//	if (debug_topol (t->prev, chain)) printf ("birth_forward\n");
  ...
  accept_likelihood_moved_branches_gibbs (chain->segment, t->prev);
  if (t->prev->prev) 
    chain->segment[t->prev->prev->last_segment]->dist = d_[0]; 
  chain->segment[t->prev->last_segment]->dist = d_[1];
  
  al_awadhi_minisampler_gibbs (chain, t->prev, afreq);
  
  /* likelihood ratio of temperatures */
  /* pi(x_star)/pi_beta(x_star) */
  sum_[0] = sum_[1] = 0.;
  gibbsPrior_dist_term_no_ratio_gibbs (&sum_[1], chain, chain->segment[t->prev->last_segment]);
  if (t->prev->prev) 
    gibbsPrior_dist_term_no_ratio_gibbs (&sum_[0], chain, chain->segment[t->prev->prev->last_segment]);
  ...
  if (chain->acceptance_probability_gibbs (chain, &acceptance, &transition)) {
    ...
    accept_add_topology_forward_gibbs (t, chain->segment);
    return;
  }
  else {
    chain->segment[t->prev->last_segment]->dist = d_[3];
    if (t->prev->prev) 
      chain->segment[t->prev->prev->last_segment]->dist = d_[2];
    reject_add_topology_forward_gibbs (t, chain->segment, chain->topol);
    return;
  }
    #ifdef DEBUG
    printf("Exiting proposal_update_topol_birth_forward_gibbs (t, chain, afreq, &log2)\n");
#endif // DEBUG
}

/*------------------------------------
  By Yujin Chung on 2/7/2013
  It replaces "proposal_update_topol_birth_backward".
---------------------------------------- */
void
proposal_update_topol_birth_backward_gibbs (topology t, chain_data_gibbs chain, acceptance_frequency *afreq, double *log2)
{
  double sum_[2], acceptance, transition, temperature = (chain->kT * chain->kT_cycles);
  int d_[4]; 
  ...
  add_topology_backward_gibbs (t, chain->segment, chain->topol, chain->split);

  /* store new and old (if rejected in the end) dist info and prior ratio terms */
  if (t->next->next) {
    d_[0] = dSPR_topology_l (t->next->next, t->next, chain->split, &(chain->max_dist_split));
    if (d_[0] > chain->max_distance) { 
      reject_add_topology_backward_gibbs (t, chain->segment, chain->topol); 
      return; }
    gibbsPrior_dist_term_awadhi_gibbs (&sum_[0], chain, chain->segment[t->next->last_segment], &d_[0], &(temperature), &(chain->kT));
    d_[2] = chain->segment[t->next->last_segment]->dist;
  }
  
  d_[1] = dSPR_topology_l (t->next, t, chain->split, &(chain->max_dist_split));
  if (d_[1] > chain->max_distance) { 
    reject_add_topology_backward_gibbs (t, chain->segment, chain->topol); return; }
  gibbsPrior_dist_term_awadhi_gibbs (&sum_[1], chain, chain->segment[t->last_segment],  &d_[1], &(temperature), &(chain->kT));
  d_[3] = chain->segment[t->last_segment]->dist;
  
  /* likelihood ratio of dimension transition */
  /* pi_beta(x')/pi(x) */
  chain->ln_likelihood_moved_branches_gibbs (chain->segment, t->next);
//	if (debug_topol (t->next, chain)) printf ("birth_backward\n");
  ...
  /* accept transient state */
  accept_likelihood_moved_branches_gibbs (chain->segment, t->next);
  if (t->next->next) 
    chain->segment[t->next->last_segment]->dist = d_[0]; 
  chain->segment[t->last_segment]->dist = d_[1];
  
  al_awadhi_minisampler_gibbs (chain, t->next, afreq);

  /* likelihood ratio of temperatures */
  /* pi(x_star)/pi_beta(x_star) */
  sum_[0] = sum_[1] = 0.;
  gibbsPrior_dist_term_no_ratio_gibbs (&sum_[1], chain, chain->segment[t->last_segment]);
  if (t->next->next) 
    gibbsPrior_dist_term_no_ratio_gibbs (&sum_[0], chain, chain->segment[t->next->last_segment]);
  ...
  if (chain->acceptance_probability_gibbs (chain, &acceptance, &transition)) {
    afreq->accept[FREQadd]++;
    accept_add_topology_backward_gibbs (t, chain->segment);
    return;
  }
  else {
    chain->segment[t->last_segment]->dist = d_[3];
    if (t->next->next) 
      chain->segment[t->next->last_segment]->dist = d_[2];
    reject_add_topology_backward_gibbs (t, chain->segment, chain->topol);
    return;
  }
}
...
/*------------------------------------
  By Yujin Chung on 2/7/2013
  It replaces "proposal_update_topol_death_forward".
---------------------------------------- */
void
proposal_update_topol_death_forward_gibbs (topology t, chain_data_gibbs chain, acceptance_frequency *afreq, double *log2)
{
  double sum_[2], acceptance, transition, temperature = (chain->kT * chain->kT_cycles);
  int d_[4]; 
  ...
  gibbsPrior_dist_term_no_ratio_gibbs (&sum_[0], chain, chain->segment[t->prev->last_segment]);
  d_[0] = chain->segment[t->prev->last_segment]->dist;
  
  sum_[0] += (sum_[1] + t->likelihood_accepted);
  acceptance = ( (temperature - chain->kT) * sum_[0] );
  
  pre_add_remove_topology_gibbs (t, chain->segment, chain->topol);
  al_awadhi_minisampler_gibbs (chain, t, afreq);
  remove_topology_forward_gibbs (t, chain->segment, chain->topol, chain->split);
  ...
  if (chain->acceptance_probability_gibbs (chain, &acceptance, &transition)) {
    ...
    accept_remove_topology_forward_gibbs (t, chain->segment, chain->topol);
    return;
  }
  else {
    if (t->next) 
      chain->segment[t->last_segment]->dist = d_[1];
    chain->segment[t->prev->last_segment]->dist = d_[0];
    reject_remove_topology_forward_gibbs (t, chain->segment, chain->topol);
    return;
  }
}
...
/*------------------------------------
  By Yujin Chung on 2/7/2013
  It replaces "proposal_update_topol_death_backward".
---------------------------------------- */
void
proposal_update_topol_death_backward_gibbs (topology t, chain_data_gibbs chain, acceptance_frequency *afreq, double *log2)
{
  double sum_[2], acceptance, transition, temperature = (chain->kT * chain->kT_cycles);
  int d_[4]; 
  ...
  gibbsPrior_dist_term_no_ratio_gibbs (&sum_[0], chain, chain->segment[t->last_segment]);
  ...
  pre_add_remove_topology_gibbs (t, chain->segment, chain->topol);
  al_awadhi_minisampler_gibbs (chain, t, afreq);
  remove_topology_backward_gibbs (t, chain->segment, chain->topol, chain->split);

  /* new dist info (t and t->next are the same) and prior ratio */
  sum_[0] = sum_[1] = 0.;
  gibbsPrior_dist_term_awadhi_gibbs (&sum_[0], chain, chain->segment[t->last_segment], &d_[2], &(chain->kT), &(temperature));
  if (t->prev) {
    gibbsPrior_dist_term_awadhi_gibbs (&sum_[1], chain, chain->segment[t->prev->last_segment], &d_[3], &(chain->kT), &(temperature));
  }
  ...
  if (chain->acceptance_probability_gibbs (chain, &acceptance, &transition)) {
    ...
    accept_likelihood_moved_branches_gibbs (chain->segment, t);
    afreq->accept[FREQremove]++;
    accept_remove_topology_backward_gibbs (t, chain->segment, chain->topol);
    return;
  }
  else {
    if (t->prev) 
    ...
    reject_remove_topology_backward_gibbs (t, chain->segment, chain->topol);
    return;
  }
}
...
/*------------------------------------
  By Yujin Chung on 2/7/2013
  It replaces "proposal_update_topol_shift_forward"
---------------------------------------- */
void
proposal_update_topol_shift_forward_gibbs (topology t, chain_data_gibbs chain, acceptance_frequency *afreq)
{
  ...
  shift_topology_forward_gibbs (t, chain->segment, chain->topol, chain->split);

  gibbsPrior_dist_term_gibbs (&sum_l, chain, chain->segment[t->prev->prev->last_segment], &d_lp);
  gibbsPrior_dist_term_gibbs (&sum_r, chain, chain->segment[t->prev->last_segment      ], &d_rp);
  chain->ln_likelihood_moved_branches_gibbs (chain->segment, t->prev);
  ...
  if (chain->acceptance_probability_gibbs (chain, &acceptance, &transition)) {
    ...
    accept_shift_topology_forward_gibbs (t, chain->segment, chain->topol);
  }
  else {
    reject_shift_topology_forward_gibbs (t, chain->segment, chain->topol);
  }
}

/*------------------------------------
  By Yujin Chung on 2/7/2013
  It replaces "proposal_update_topol_shift_backward".
---------------------------------------- */
void
proposal_update_topol_shift_backward_gibbs (topology t, chain_data_gibbs chain, acceptance_frequency *afreq)
{	
  double sum_l = 0., sum_r = 0., acceptance, transition;
  int d_lp = chain->segment[ t->last_segment ]->dist, d_rp = 0; 
  afreq->propose[FREQshift]++;
  
  shift_topology_backward_gibbs (t, chain->segment, chain->topol, chain->split);

  gibbsPrior_dist_term_gibbs (&sum_l, chain, chain->segment[t->last_segment      ], &d_lp);
  gibbsPrior_dist_term_gibbs (&sum_r, chain, chain->segment[t->next->last_segment], &d_rp);
  chain->ln_likelihood_moved_branches_gibbs (chain->segment, t->next);
  ...
  if (chain->acceptance_probability_gibbs (chain, &acceptance, &transition)) {
    ...
    afreq->accept[FREQshift]++;
    accept_shift_topology_backward_gibbs (t, chain->segment, chain->topol);
  }
  else {
    reject_shift_topology_backward_gibbs (t, chain->segment, chain->topol);
  }
}

/*------------------------------------
  By Yujin Chung on 2/7/2013
  It replaces "debug_topol".
---------------------------------------- */
bool
debug_topol_gibbs (topology t, chain_data_gibbs chain)
{
  double old_lik = t->likelihood_proposal;
  int i;
  /*check likelihood */
  ln_likelihood_gibbs (chain->segment, t);
...
/*------------------------------------
  By Yujin Chung on 2/7/2013
  All the below functions are from biomc2-1.9
---------------------------------------- */
