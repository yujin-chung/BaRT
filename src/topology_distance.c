...
int RF_partition_l (split_space split, int *max);
...
void create_subtree_bipartition_l (topol_node this, int *n_leaves)
{
	...
	split_OR_l (&(this->bipartition), &(this->left->bipartition), &(this->right->bipartition), n_leaves);
}
...
int dSPR_topology_l (topology t1, topology t2, split_space split, int *max)
{
    ...
    #ifdef DEBUG
  printf("\n[YC 9/9/2016] dSPR_topology_l() is called.\n This function is for computing the SPR distance between two topologies. \n\n");

  char *tr1 = topology_to_string (t1);
  char *tr2 = topology_to_string (t2);
  printf("t1 = %s, t2 = %s\n",tr1,tr2);
#endif //DEBUG
  
	int i;
  #ifdef DEBUG
	//  printf("REMOVE: Calling initialize_split_space_l (split);\n");
#endif //DEBUG  
  initialize_split_space_l (split);
	...
	// return dSPR_partition_l (split, max);
   int dist = RF_partition_l (split, max);
   #ifdef DEBUG
   printf("RF distance = %d\n",dist);
   printf("Exiting dSPR_topology_l\n");
#endif //DEBUG
   return dist;
}
...
int dSPR_partition_l (split_space split, int *max)
{
	...
	bool verbose = 0; /* DEBUG option so that we can see the steps of d_SPR calc */
...
//		qsort ((split->u2), split->n_u2, sizeof (split_partition*), compare_split_partition_l);
		...
		/* TEMP: RF-distance */
//		return (int) (((double)(split->n_u1 + split->n_u2))/2.);
...
//		qsort ((split->u2), split->n_u2, sizeof (split_partition*), compare_split_partition_l);
		...
		/* TEMP: RF-distance */
		return (int) (((double)(split->n_u1 + split->n_u2))/2.);
		// This is the existing code of biomc2 // YC 5/24/2017
