...
void topology_subtree_to_string_with_name_generate_length (char *str, const topol_node node, char **taxlabel, double *min_length, double *max_length);
...
topology new_topology (int nleaves) 
{
  ...
  tree = (topology) biomc2_malloc (sizeof (struct topology_struct));
  ...
  for (i=0; i<tree->nleaves; i++) { 
    ...
    split_initialize_l (&(tree->nodelist[i]->bipartition), i, &(tree->nleaves));
