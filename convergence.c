...
/*-----------------------------------
  ...
  It replaces "prepare_convergence".
-------------------------------------- */
convergence
prepare_convergence_gibbs (chain_data_gibbs chain)
{
  int i=0;
  topology t;
...
/*-----------------------------------
  By Yujin Chung on 2/6/2013:
  This function is from biomc2-1.9
-------------------------------------- */
convergence
...
/*-----------------------------------
  By Yujin Chung on 2/6/2013:
  It replaces "update_convergence"
-------------------------------------- */
void
update_convergence_gibbs (chain_data_gibbs chain, convergence C)
{
  topology t;
  int current = 0;
  ...
  for (t=chain->topol->first; (t) && (current < C->n_t); ) {
    if (t->last_segment >= C->position[current]) {
      /* topologies */
      // YC 9/9/2016
      // dSPR_topology_l(): Calculate approximate \f$d_{SPR}\f$
      //                    between topologies t1 and t2.
