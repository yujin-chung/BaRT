  ...
  if (tsp) {
    int i,j;
      ...
      for (i=tsp->ntrees-1; i>=0; i--) if (tsp->T[i]) {
        /* tree taxlabels are removed here */
        /* // BUG FIXED 2008.08.14: no need to free here, since they are only pointers (initialized only once) 
        for (j=tsp->nnodes-1; j >=0; j--) 
           if ((tsp->T[i]->nodelist[j]) && (tsp->T[i]->nodelist[j]->taxlabel)) free (tsp->T[i]->nodelist[j]->taxlabel);
         */
