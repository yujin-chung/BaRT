...
/*---------------------------------------
  ...
  because of new data type phylogeny_gibbs
-------------------------------------------*/
void          update_likelihood_gibbs (phylogeny_gibbs *p, topol_node this, topology t);
void update_accepted_likelihood_gibbs (phylogeny_gibbs *p, topol_node this, topol_node upnode, topology t);
void          update_likelihood_moved_branches_gibbs (phylogeny_gibbs *p, topol_node this, topology t);
void update_accepted_likelihood_moved_branches_gibbs (phylogeny_gibbs *p, topol_node this, topol_node upnode, topology t);
void          update_likelihood_proposal_gibbs (phylogeny_gibbs p, topol_node this);
void update_accepted_likelihood_proposal_gibbs (phylogeny_gibbs p, topol_node this, topol_node upnode);
void mark_d_done_gibbs (phylogeny_gibbs *p, topol_node this, topology t);
void mark_d_done_moved_branches_gibbs (phylogeny_gibbs *p, topol_node this, topology t);

/*---------------------------------------
  By Yujin Chung on 2/6/2013
  The functions below are from biomc2-1.9
-------------------------------------------*/
...
/*-------------------------------------------
  By Yujin Chung on 2/6/2013:
  It replaces "update_Q_eigenvalues_gibbs".
------------------------------------------------ */
void
update_Q_eigenvalues_gibbs (phylogeny_gibbs p, double kappa)
{
...
void update_Q_matrix_gibbs (phylogeny_gibbs p, double lambda)
{
  ...
  for (i=0; i < 4; i++) {
    for (j=0; j < 4;j++) {
      p->Q[j][i] = 0.;
      for (k=0; k < 4; k++) 
	p->Q[j][i] += (p->z1[k][i] * p->z2[k][j])/(1. + (p->psi[k] * lambda));      
    }
  }
}
...
void ln_likelihood_gibbs (phylogeny_gibbs *p, topology t)
{
  ...
  update_likelihood_gibbs (p, t->root->left, t);
...
/*------------------------
  By Yujin Chung on 2/6/2013:
  It replaces "update_likelihood".
---------------------------------- */
void
update_likelihood_gibbs (phylogeny_gibbs *p, topol_node this, topology t)
{
  int i;
  if (this->left->internal)  
    update_likelihood_moved_branches_gibbs (p, this->left, t);
  if (this->right->internal) 
    update_likelihood_moved_branches_gibbs (p, this->right, t);
...
/*----------------------------------------------------
  By Yujin Chung on 2/6/2013:
  It replaces "accept_likelihood".
---------------------------------------------------- */
void 
accept_likelihood_gibbs (phylogeny_gibbs *p, topology t)
{
  int i;
  ...
  mark_d_done_gibbs (p, t->root->left, t);

  if (t->root->left->left->internal) 
    update_accepted_likelihood_gibbs (p, t->root->left->left, t->root, t);
  if (t->root->left->right->internal) 
    update_accepted_likelihood_gibbs (p, t->root->left->right, t->root, t);
}	


/*-----------------------------
  By Yujin Chung on 2/6/2013:
  It replaces "update_accepted_likelihood".
--------------------------------- */
void
update_accepted_likelihood_gibbs (phylogeny_gibbs *p, topol_node this, topol_node upnode, topology t)
{
  int i;
  ...
  if (this->left->internal)  
    update_accepted_likelihood_gibbs (p, this->left, this, t);
  if (this->right->internal) 
    update_accepted_likelihood_gibbs (p, this->right, this, t);
}

/*-------------------------------------------
  By Yujin Chung on 2/6/2013:
  It replaces "ln_likelihood_moved_branches"
---------------------------------------------- */
void
ln_likelihood_moved_branches_gibbs (phylogeny_gibbs *p, topology t)
{
  int i;
  ...
  while ((ingroup->internal) && (ingroup->u_done)) {
    if (!ingroup->left->d_done) 
      ingroup = ingroup->left;
    else  
      ingroup = ingroup->right;
  }
  if (ingroup != t->root->left) 
    ingroup = ingroup->up;

  if (!(ingroup->d_done))
    update_likelihood_moved_branches_gibbs (p, ingroup, t);
  else {
    for (i=t->first_segment; i <= t->last_segment; i++) {
...
/*---------------------------------
  By Yujin Chung on 2/6/2013:
  It replaces "update_likelihood_moved_branches".
------------------------------------- */
void
update_likelihood_moved_branches_gibbs (phylogeny_gibbs *p, topol_node this, topology t)
{
  int i;
...
/*---------------------------------------------
  By Yujin Chung on 2/6/2013:
  It replaces "accept_likelihood_moved_branches".
------------------------------------------------ */
void 
accept_likelihood_moved_branches_gibbs (phylogeny_gibbs *p, topology t)
{
  int i;
  topol_node ingroup = t->root->left;
  //	bool debg = false;
  ...
  if (!(ingroup->d_done)) {
    if (ingroup->internal) 
      mark_d_done_moved_branches_gibbs (p, ingroup, t);
    if (ingroup != t->root->left) {
      update_likelihood_moved_branches_gibbs (p, t->root->left, t);
      mark_d_done_moved_branches_gibbs (p, t->root->left, t);
    }
  }
  
  t->root->left->u_done = true;
  if (t->root->left->left->internal) 
    update_accepted_likelihood_moved_branches_gibbs (p, t->root->left->left, t->root, t);
  if (t->root->left->right->internal) 
    update_accepted_likelihood_moved_branches_gibbs (p, t->root->left->right, t->root, t);
  /*
...
/*---------------------------------------
  By Yujin Chung on 2/6/2013:
  It replaces "update_accepted_likelihood_moved_branches"
------------------------------------------------- */
void
update_accepted_likelihood_moved_branches_gibbs (phylogeny_gibbs *p, topol_node this, topol_node upnode, topology t)
{
  int i;
  ...
  this->u_done = true;
  if (this->left->internal)  
    update_accepted_likelihood_moved_branches_gibbs (p, this->left, this, t);
  if (this->right->internal) 
    update_accepted_likelihood_moved_branches_gibbs (p, this->right, this, t);
}


/*-------------------------------
  By Yujin Chung on 2/6/2013:
  It replaces "ln_likelihood_proposal".
----------------------------------*/
void
ln_likelihood_proposal_gibbs (phylogeny_gibbs p, topology t)
{
  p->likelihood_proposal = 0.;
  
  update_likelihood_proposal_gibbs (p, t->root->left);
...
/*---------------------------------------------
  By Yujin Chung on 2/6/2013:
  It replaces "update_likelihood_proposal".
----------------------------------------------- */
void
update_likelihood_proposal_gibbs (phylogeny_gibbs p, topol_node this)
{
  if (this->left->internal)
    update_likelihood_proposal_gibbs (p, this->left);
  if (this->right->internal) 
    update_likelihood_proposal_gibbs (p, this->right);
...
/*---------------------------------------
  By Yujin Chung on 2/6/2013:
  It replaces "accept_likelihood_proposal".
----------------------------------------- */
void 
accept_likelihood_proposal_gibbs (phylogeny_gibbs p, topology t)
{
  int i;
  ...
  if (t->root->left->left->internal) 
    update_accepted_likelihood_proposal_gibbs (p, t->root->left->left, t->root);
  if (t->root->left->right->internal) 
    update_accepted_likelihood_proposal_gibbs (p, t->root->left->right, t->root);
}	

/*-----------------------------------
  By Yujin Chung on 2/6/2013:
  It replaces "update_accepted_likelihood_proposal"
------------------------------------- */
void
update_accepted_likelihood_proposal_gibbs (phylogeny_gibbs p, topol_node this, topol_node upnode)
{
  ...
  if (this->left->internal)  
    update_accepted_likelihood_proposal_gibbs (p, this->left, this);
  if (this->right->internal) 
    update_accepted_likelihood_proposal_gibbs (p, this->right, this);
}

/*--------------------------------
  By Yujin Chung on 2/6/2013:
  It replaces "mark_d_done".
---------------------------------- */
void
mark_d_done_gibbs (phylogeny_gibbs *p, topol_node this, topology t)
{
  int i;
  if (this->left->internal)  
    mark_d_done_gibbs (p, this->left, t);
  if (this->right->internal) 
    mark_d_done_gibbs (p, this->right, t);
...
/*--------------------------------
  By Yujin Chung on 2/6/2013:
  It replaces "mark_d_done_moved_branches".
---------------------------------- */
void
mark_d_done_moved_branches_gibbs (phylogeny_gibbs *p, topol_node this, topology t)
{
  int i;
  if ((this->left->internal) && !(this->left->d_done)) {
    mark_d_done_moved_branches_gibbs (p, this->left, t);
  }
  if ((this->right->internal) && !(this->right->d_done)) {
    mark_d_done_moved_branches_gibbs (p, this->right, t);
  }
